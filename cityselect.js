(function ($) {
    ...
    function initHtml(options) {
        ...
        html += '<div class="province-wrapper"><div class="nav flex"><a tb="province-con" id="province-nav" class="current">省份</a><a tb="city-con" id="city-nav">城市</a><a tb="county-con" id="county-nav">区/县</a></div>';
        ...
        if (direction == "right") {
            $(".province-wrapper").css({
                "right": winWidth - elWidth - elOffsetLeft,
                "top": elOffsetTop + elHeight + 8
            })
        } else {
            $(".province-wrapper").css({
                "left": elOffsetLeft,
    ...
    function initEvent(options) {
        ...
        $(".province-wrapper .nav a").on("click", function () {
            ...
            if ($(this).attr("tb") == "county-con" && $(".city-con .list .current").length < 1) {
                return;
            }
            $(this).addClass("current").siblings().removeClass("current");
            $(".province-wrapper").find("." + $(this).attr("tb")).show().siblings().hide();
        })
    }
    ...
    function getProvinceCityCounty(el, provinceCityCounty, cb) {
        ...
        if (elVal) {
            ...
            var cityName = elVal[1];
            var cityId;
            var cityIndex;
            if (provinceId) {
                var allCity = allCityMap.get(provinceId);
                $.each(allCity,
                    function (i) {
    ...
    function buildList(start, end, el, provinceCityCounty, text, textAll, cb) {
        ...
        for (var i = start; i < end; i++) {
            if (text == "province") {
                textName = textAll[i].name;
                textId = textAll[i].id;
                if (textName == '内蒙古自治区') {
                    textShortName = '内蒙古';
                } else if (textName == '黑龙江省') {
                    textShortName = '黑龙江';
                } else {
                    textShortName = textName.substr(0, 2);
                }

            } else {
                textName = textAll[i].name;
                textShortName = textName.substr(0, 4);
                textId = textAll[i].id
            }

            html += '<li id="' + textId + '" title="' + textName + '">' + textShortName + '</li>';
        }

        if (text == "province") {
            $(".province-con .list ul").html(html);
